...
| 단계 | 기간      | 주요 작업                    |
|------|---------|--------------------------|
| **1단계** | 1개월     | 채팅 서버 개발 (TCP Socket 서버) |
...
### 📌 개발 순서 요약
1️⃣ **채팅 서버 구축** (TCP 서버, 글로벌 채팅, DM, 파티 채팅)  
...
### **1️⃣ 채팅 서버 및 기본 네트워크 인프라 구축**
🎯 **목표:** 클라이언트-서버 통신을 연습하고, 실시간 메시지 브로드캐스트를 구현  
### 🔧 TCP 채팅 서버 구축
- [x]  C++ TCP 서버 기본 코드 작성
- [x]  클라이언트-서버 간 단일 메시지 송수신 테스트
- [x]  클라이언트별 접속 식별 구조 설계 (IP + 포트 또는 UUID)
- [x]  Broadcast 기능 구현 (Global Chat)
- [x]  귓속말 명령어 파싱 (`/w`, `/dm` 처리)
### 🔧 클라이언트 연동
- [x]  Unity 클라이언트에서 TCP 메시지 송신 구현
- [x]  수신 메시지 UI 반영(Global/DM/Party 분리)
- [x]  서버 응답 처리 구조 구성
