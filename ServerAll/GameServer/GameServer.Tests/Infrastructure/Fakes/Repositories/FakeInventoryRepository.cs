...
{
    private readonly Dictionary<(long UserId, int ItemId), InventoryItem> _items = new();
    ...
    public Task<InventoryItem> AddQuantityAsync(long userId, int itemId, int amount, int maxStack, CancellationToken ct = default)
    {
    ...
    public Task<int?> RemoveQuantityAsync(long userId, int itemId, int amount, CancellationToken ct = default)
    {
        if (amount <= 0)
