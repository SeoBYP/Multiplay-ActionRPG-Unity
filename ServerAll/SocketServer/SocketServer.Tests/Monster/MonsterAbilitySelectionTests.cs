...
public class MonsterAbilitySelectionTests
{
    ...
    public void 사거리_밖이면_해당_어빌리티는_발동하지_않는다()
    {
        ...
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 3f, 0f, 0f, 0f); // aggro 안(7) · 공격 사거리 밖(1.3)
        room.MarkJoined(100);
    ...
    public void 쿨다운은_어빌리티_데이터의_값을_따른다()
    {
        ...
        Assert.Single(room.Tick(0.1f, t0).OfType<S_AbilityActivated>());              // 첫 발동
        Assert.Empty(room.Tick(0.1f, t0 + cd - 1).OfType<S_AbilityActivated>());      // 쿨다운 1ms 전 → 거부
        Assert.Single(room.Tick(0.1f, t0 + cd).OfType<S_AbilityActivated>());         // 경계 = 발동
    }
    ...
    public void 데미지와_CC는_어빌리티_저작값을_따른다()
    {
        ...
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 0.5f, 0f, 0f, 0f, attackPower: 0, defense: 0);
        room.MarkJoined(100);
        Spawn(room, "arachnya");
