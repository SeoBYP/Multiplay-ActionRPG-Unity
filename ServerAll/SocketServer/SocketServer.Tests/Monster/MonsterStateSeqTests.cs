...
public class MonsterStateSeqTests
{
    ...
    {
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 3f, 0f, 0f, 0f); // Chase = 매 틱 dirty
        room.MarkJoined(100);
        SpawnCreepyDemon(room);

        var first = room.Tick(0.1f, 1_000_000).OfType<S_MonsterState>().Single();
    ...
    {
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 3f, 0f, 0f, 0f); // Chase = 매 틱 송신
        room.MarkJoined(100);
        SpawnCreepyDemon(room);

        int s1 = room.Tick(0.1f, 1_000_000).OfType<S_MonsterState>().Single().Seq;
        int s2 = room.Tick(0.1f, 1_000_100).OfType<S_MonsterState>().Single().Seq;
        int s3 = room.Tick(0.1f, 1_000_200).OfType<S_MonsterState>().Single().Seq;
    ...
    {
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 3f, 0f, 0f, 0f);
        room.MarkJoined(100);
        room.SpawnMonsters(
        ...
        var states = room.Tick(0.1f, 1_000_000).OfType<S_MonsterState>().ToList();
    ...
    public void 데미지_스냅샷이_틱_스냅샷보다_나중이면_Seq가_더_크다_순서역전_무효화()
    {
        ...
        var room = NewRoom();
        room.AddPlayer(100, "A", 0, 3f, 0f, 0f, 0f); // Chase = 틱이 매번 송신
        room.MarkJoined(100);
        SpawnCreepyDemon(room);

        // ① 틱이 패킷 생성(= 이 시점 HP 스냅샷). RoomTickService 는 아직 안 보냈다고 가정.
        var tickPacket = room.Tick(0.1f, 1_000_000).OfType<S_MonsterState>().Single();
        ...
        var (hit, newHp, _) = room.Actors.DamageMonster(monster.InstanceId, dmg);
        Assert.True(hit);
