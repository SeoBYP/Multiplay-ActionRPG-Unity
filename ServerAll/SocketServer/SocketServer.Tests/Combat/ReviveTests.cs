...
public class ReviveTests
{
    ...
    {
        var room = NewRoom();
        room.AddPlayer(100, "R", 0, 0f, 0f, 0f, 0f, maxHealth: 200); // 시전자
        room.AddPlayer(200, "T", 1, targetX, 0f, 0f, 0f, maxHealth: 200); // 대상
        room.MarkJoined(100);
        room.MarkJoined(200);
        room.Progress.ApplyPlayerEffect(200, Lethal()); // 대상 HP0 → 다운
        return room;
    }
    ...
    public void 부활은_다운된_아군의_HP를_50퍼센트_복구하고_다운에서_제거한다()
    {
        ...
        Assert.True(room.Actors.GetMember(200)!.Actor.Gas.IsDead);

        var (ok, hp) = room.Progress.TryRevive(100, 200);

        Assert.True(ok);
        Assert.Equal(100, hp); // MaxHp 200 × 50%
        Assert.Equal(100, room.Actors.GetMember(200)!.Actor.Gas[EGameplayAttribute.Health]);
        Assert.False(room.Actors.GetMember(200)!.Actor.Gas.IsDead);
    }
    ...
    public void 거리가_사거리_밖이면_부활은_거부된다()
    {
        ...
        var (ok, _) = room.Progress.TryRevive(100, 200);

        Assert.False(ok);
        Assert.True(room.Actors.GetMember(200)!.Actor.Gas.IsDead); // 여전히 다운
    }
    ...
    public void 다운되지_않은_대상이거나_이미_부활됐으면_거부된다_멱등()
    {
        ...
        var first = room.Progress.TryRevive(100, 200);
        Assert.True(first.Ok);

        // 두 번째 — 이미 부활(다운 아님) → 멱등 거부.
        var second = room.Progress.TryRevive(100, 200);
    ...
    {
        var room = DownedTargetRoom();
        Assert.False(room.Progress.TryRevive(200, 200).Ok); // reviver==target
    }
}
