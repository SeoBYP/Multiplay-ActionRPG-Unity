...
{
    private static PlayerActor Player(long id, float x, float z, float rotY = 0f)
        => new(id) { PosX = x, PosY = 0f, PosZ = z, RotY = rotY };

    // 아군 오사 폐지(2026-08-22)로 플레이어 hitbox 적중 판정(SelectHitTargets)은 제거됐다.
    // 근거: 그 경로는 클라 HP 만 깎고 서버 PlayerActor 의 HP 를 안 건드려, 파티가 붙어 싸우면
