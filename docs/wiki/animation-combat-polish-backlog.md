...
> 맥락: MotionMatching 외부화 후 클라는 **단순 PROTOFACTOR Animator**로 복원됨. 2026-07 세션에서 플레이어·원격 플레이어 애니 기반 + 무기 판정 + HitboxMath 방향 버그 + 이동잠금 + 부활 애니까지 세움(codemap §2.52~2.56). 이 문서는 **그 위에 남은 폴리시 항목**을 실행 가능하게 목록화한다.
> 관련: [codemap.md](codemap.md) §2.52~2.56 · [plan.md](plan.md) · [.claude/rules/unity-gameplay-state.md](../../.claude/rules/unity-gameplay-state.md)
...
| 3 | 부활 기상(GetUp) 클립 | 소 | 없음 | 2 |
| 4 | Interact 전용 클립 | 소 | 없음 | 2 |
