...
| **서버 액터 모델(캐릭터·전투 단일 표현)** | `SocketServer/Actors/{Actor,PlayerActor,MonsterActor}.cs` + `Shared.Gameplay/Actors/AbilitySystemComponent.cs`(HP·마나·스탯·태그·활성 Effect·쿨다운 — **클라 `GasComponent`(MonoBehaviour)도 이 타입을 소유해 위임**) · 방 관리는 `SocketServer/Room/RoomMember.cs` 분리 · 저장소=`Room._actors`(ActorId 키) 단일. §2.81·2.86 | [actor-combat-architecture.md](actor-combat-architecture.md) |
...
기능 자체가 아니라 **좌표의 출처**가 바뀌기 때문이다.

```
[전]  Main 진입 → 원점 스폰(고정)                   클라가 좌표를 정할 여지 0
[후]  플레이 중 ─클라가 만든 좌표─▶ 서버 저장 ─▶ 복원 시작점
                                     ▲ 여기서 무엇을 믿을지 정해야 한다
```

F1(로비 원자성)이 "기능을 다 만든 뒤 신뢰 경계를 붙였다가 결함으로 되돌아온" 사례라, 이번엔 순서를 뒤집었다.

**② 착수 전 실측 — 검증 재료가 백로그 기술과 달랐다**

| 백로그가 든 검증 후보 | 실제 |
|---|---|
| 맵 경계 | ✅ 있다. `MapBounds` 가 `spawn-layouts.json` 에 bake 되고 `SpawnLayoutTable` 이 읽는다 |
| 내비메시 | ❌ 서버에 없다(클라 자산) |
| 진입 게이트 | ❌ **시스템 자체가 없다** — `RequiredLevel`·`minLevel` 류 검색 0건 |

→ 백로그가 든 "게이트 걸린 구역 우회"는 **아직 존재하지 않는 위험**이다. 지금 방어할 대상이 없다.
또 하나 정정: Main 플레이어 스폰은 "저작 고정값"이 아니라 **원점**이었다(`CharacterSpawner` 가
`SocketState != Joined` 면 `Vector3.zero`). 그래서 복원 실패 시 폴백이 곧 기존 동작이다.

**③ 결정 3가지(사용자, 2026-08-25)와 그 구현**

```
저장   클라(이동 2m && 5s 경과 시에만) ──▶ 서버
                                          ├ mapId 가 카탈로그에 있나  ─없으면─▶ 거부
                                          ├ MapBounds 안인가          ─밖이면─▶ 최근접 저작 스폰으로 스냅
                                          └ Redis Hash (TTL 24h)
확정   로그아웃 · 던전 입장 ──▶ FlushAsync ──▶ user_positions UPSERT
복원   Main 진입 ──▶ Redis → DB → (없음)
                 ──▶ 클라: NavMesh.SamplePosition 지면 스냅 → 실패하면 원점
```

- **서버는 경계만, 클라는 지면만** 검증한다 — 각 계층이 아는 것만 본다. 내비메시를 서버에 올리는 과설계 회피.
- 경계 밖을 **clamp 하지 않고 저작 스폰으로 스냅**한다: clamp 는 경계선 위 임의 점이라 지형 밖·벽 안일 수 있지만,
  저작 스폰은 사람이 "여기서 시작해도 된다"고 찍은 점이다.
- **고정 주기가 아니라 "움직였을 때만"**(2m + 5s). 가만히 선 유저의 트래픽이 0 이 된다.
- 던전 왕복은 별도 로직이 없다 — Main 을 떠날 때(StartGame) 확정 저장하므로 "입장 직전 자리"가 자연히 성립한다.

**④ 하지 않은 것 (명시)**

- **이동 궤적·근접 검증**. 검증 대상인 좌표 자체가 클라가 만든 값이라 순환이다(F5 와 같은 결론).
  NPC 좌표를 bake 해도 마찬가지다 — 위치를 위조할 수 있는 자는 근접도 위조한다.
  제대로 하려면 연속 보고 간 **속도 검증**까지 가야 하고, 그건 스로틀 설계부터 다시 잡는 별개 기능이다.
- 해소 조건은 여전히 **Main 의 서버 권위 승격**이다.

**⑤ 교리 예외 — 이 도메인만 Redis 가 1차 저장소다**

`networking.md` 는 "Update 는 캐시를 덮어쓰지 않는다(DB 저장 후 DEL)" 인데, 위치는 쓰기가 매우 잦고
유실이 허용되는 유일한 데이터라 **Redis 에 쓰고 이탈 시점에만 DB 확정**한다. 유실 폭이
"마지막 확정 이후"로 한정되고 명시적이다. 다른 도메인에 복사하지 말 것 — 규칙 파일에 사유와 함께 적었다.

**변경 위치**: proto `user.proto`(+2 RPC) · Domain `UserPosition` · Application `IUserPositionService`/`UserPositionService` ·
Infrastructure `UserPositionRepository`+EF설정+마이그레이션 · API `UserGrpcService` · Flush 2곳(`AuthService.LogoutAsync`,
`DungeonLobbyService.StartGameAsync`) · 클라 `IPlayerPositionService`/`PlayerPositionService`/`MainPositionReporter`/`CharacterSpawner`.

**검증**: 서버 빌드0 · **서버 738/738**(733→738, 신규 단위 6 + 통합 5) · 클라 컴파일0 ·
**PlayMode 228/228** · **EditMode 239/239** · 마이그레이션 실환경 적용 확인(`user_positions` PK/컬럼 7종).

⚠️ **미실측**: 사람이 실제로 Main 에서 이동 → 재접속해 그 자리에서 시작하는 것은 보지 못했다.
클라 복원 분기(`NavMesh.SamplePosition`)는 베이크된 내비메시가 있는 씬이 필요해 자동 테스트로 덮지 못했다.

### 2.114 ReportTalk — 근접 검증이 불가능함을 확정하고, 대신 "정상 요청만" 오게 했다 (F5, 2026-08-25)

**① 백로그의 전제가 틀렸다 (실측)**

F5 는 "서버가 NPC 위치를 알고 있으므로 근접 검증을 넣으면 된다"고 적혀 있었다. 둘 다 사실이 아니다.

| 필요한 것 | 실제 |
|---|---|
| NPC 위치 | 서버에 **없다**. NPC 는 `NPCDialogueInteractable`(MonoBehaviour, `[SerializeField] npcId`)로 씬에 배치되고 위치 카탈로그가 없다. 서버가 아는 건 `quests.json` 의 `targetId` 문자열뿐 |
| 플레이어 위치 | 서버가 **모른다**. Main 씬은 소켓 미연결(`SocketState != Joined`) — 위치를 보낼 채널 자체가 없다 |

→ **근접 검증은 Main 을 서버 권위로 올리기 전에는 성립하지 않는다.** 대조군으로 인용되던 `ClaimMonsterExp` 도
사실 위치를 검증하지 않는다 — `SpawnLayoutTable` 에 `(mapId, slotId)` 존재 확인 + Redis 쿨다운으로 **파밍률 상한**을 걸 뿐이다.
즉 이 레포가 Main 에서 쓰는 교리는 "위치 증명"이 아니라 "카탈로그 존재 + 레이트 상한"이다.

**② 그래서 목표를 바꿨다 — 치팅 차단이 아니라 "정상 요청만 오게 한다"**

사용자 결정(2026-08-25): 대화 진행 치팅은 운영상 검증 대상이 아니다. 대신 불필요한 왕복을 없앤다.

```
대화 시작
  ① NPC.hasQuest == false ─────────────▶ 통신 0회로 끝            ← 저작 플래그(조기 차단)
  ② true → GetQuests 로 상태 갱신
  ③ 이 npcId 대상 TalkToNpc 가 Accepted ─아니면▶ ReportTalk 생략   ← 실제 판단(서버가 준 상태)
  ④ 맞으면 ReportTalk → 성공 시 상태 재갱신(Accepted→Completed 반영)
```

- **판단 근거를 이중화한 이유**: `hasQuest` 만 믿으면 플래그를 `false` 로 잘못 둔 NPC 의 퀘스트가 조용히 죽는다.
  그래서 플래그는 **조기 차단 전용**이고, 보고 여부는 서버가 내려준 퀘스트 상태(`QuestInfo.target_id`·`status`)가 정한다.
  proto 는 이미 `target_id`(51행)를 내려주고 있어 **계약 변경이 없다**.
- ⚠ 남는 드리프트: `hasQuest=false` 인 NPC 에 퀘스트를 새로 붙이면 그 퀘스트가 진행되지 않는다.
  이 실패는 "호출이 아예 안 오는" 형태라 서버 로그로도 안 잡힌다. 툴팁에 경고를 달았고 backlog 에 기록했다.
- 순서를 바꿨다(예전엔 `ReportTalk` → `Refresh`). 이제 잡담 NPC 는 **0회**, 수주 안 한 퀘스트 NPC 는 **1회**,
  실제 진행이 일어나는 대화만 **3회**다.

**③ 서버는 "처리 가능한 요청인가"만 본다**

`QuestService.ReportTalkAsync` 가 `AdvanceMatchingAsync` 를 바로 부르던 것을 앞단 검증으로 감쌌다.

- 카탈로그에 이 npcId 를 대상으로 하는 `TalkToNpc` 정의가 없으면 **DB 를 읽지 않고** 0 반환 + 경고 로그.
  예전에는 `GetAllForUserAsync` 를 **먼저** 호출한 뒤 매칭해서, 임의 문자열 스팸이 그대로 저장소 왕복이었다.
- 실패를 예외/Failure 로 만들지 않았다 — 퀘스트 없는 NPC 와 대화하는 것은 **정상 행동**이고,
  클라는 대화창을 열 때 이 RPC 를 부른다. 실패로 만들면 잡담 NPC 가 에러가 된다. proto 무변경.
- `ReportTalk` 은 로그가 0건이었다. 진행/거부를 구분해 남겨 비정상 경로가 보이게 했다.

**④ 하지 않은 것**

per-(user,npc) 쿨다운을 검토했다가 **뺐다**. 실측상 `TalkToNpc` 퀘스트는 1개(`quest_greet_elder`, `requiredCount=1`)라
막을 파밍이 존재하지 않는다. `RequiredCount > 1` 대화 목표가 생기면 그때 넣는다(YAGNI).

**검증**: 서버 빌드0 · **서버 728/728**(724→728, GameServer 443→446 + 기존 1건 재작성) ·
클라 컴파일0 · **PlayMode 228/228**(225→228) · **EditMode 239/239**.
...
- **증상**: 매달린 팔 간격이 애니메이션과 다르게 벌어졌다. **원인**: IK 가 손 위치를 <b>기둥 좌표로 통째로 덮어썼다</b>
  (좌우를 `m_railHalfWidth` ±0.33 으로 고정) → 클립이 가진 어깨너비 기준 간격이 사라진다.
- **해결(축을 나눠서 필요한 것만 고친다)**: 클립 손 위치를 사다리 좌표계로 분해해
  <b>좌우(팔 간격)는 클립 값 그대로</b> 두고 <b>깊이(면까지)와 높이(가장 가까운 발판)만</b> 보정한다.
  실제로 어긋나는 축은 이 둘뿐이다(발판 간격 0.6m 는 클립 제작 기준과 다르다).
- **적용 시점**: 속도비 판정 → <b>거리(접촉) 판정</b>으로 교체. 클립 손이 보정 목표에서 `m_contactRadius`(0.28m) 밖이면 가중치 0
  = 뻗는 중·당기는 중엔 클립 그대로, <b>닿는 순간에만</b> 스냅. 사용자 요구("팔 뻗어서 사다리에 닿을 때만")와 1:1.
- **파일**: `Gameplay/Character/LadderIK.cs`(`ResolveGrip`/`ContactWeight` 순수함수로 분리해 테스트 가능하게) ·
  `Gameplay/Character/Interactions/Ladder.cs`(`GetFaceAxes`/`GetApproachSide` public — IK 가 면 축을 써야 한다).
- **덤(플레이크 수정)**: `E2ETestBase.UniqueNickname` 이 밀리초 하위 6자리만 써서 다중 클라 E2E 가 같은 ms 에 등록하면
  `NICKNAME_ALREADY_TAKEN` 으로 산발 실패했다 → 시간(5)+프로세스 일련번호(1)+난수(4) 로 교체.
- **검증**: **ClimbTests 10/10**(신규 2건: 팔 간격 유지·접촉 시에만 가중치) · **EditMode 213/213** · **PlayMode 202/202**(249.1s, 플레이크 수정 후 무실패).
- **미실측**: `m_handGripDepth` 0.06 / `m_footGripDepth` 0.12 / `m_contactRadius` 0.28 은 눈으로 맞출 값.

**P15 — 사다리 IK 정정: 좌우는 캐릭터 기준 · 접지 구간에만 적용 (2026-08-21)**
- **증상 ①**: 한쪽 손발은 맞고 반대쪽은 뒤집혔다. **원인**: `GetGripPoint` 가 좌우를 <b>사다리 고정 축</b>(폭 축)으로 계산했다 —
  캐릭터가 반대편 면에 서면 캐릭터의 왼쪽이 그 축의 오른쪽이 되어 정확히 절반만 맞는다.
  **해결**: 그립이 캐릭터의 `right` 를 받아 그 기준으로 좌우를 정한다. 실측: 앞/뒤 면 모두 오른손 +0.33 · 왼손 −0.33(캐릭터 오른쪽 성분).
- **증상 ②**: 네 팔다리가 늘 고정돼 오르는 동작이 죽었다. **원인**: `sensor.Current != null` 이면 항상 IK 를 걸었다.
  **해결**: <b>붙잡은 팔다리에만</b> 건다. 판정은 클립에서 직접 뽑는다 — 붙잡은 팔다리는 월드에서 거의 정지(몸만 상승),
  뻗는 팔다리는 몸보다 빠르게 이동 → 속도비로 팔다리별 가중치(별도 커브 저작 불필요, `m_reachSpeedRatio` 로 튜닝).
- **파일**: `Gameplay/Character/Interactions/Ladder.cs`(GetGripPoint 시그니처에 characterRight 추가) · `Gameplay/Character/LadderIK.cs`(전면 재작성).
- **검증**: 좌우 매핑 실측(앞/뒤 면 모두 오른손 +0.33 · 왼손 −0.33) · **ClimbTests 8/8** · **EditMode 213/213 · PlayMode 200/200**(254.7s).
- **미실측**: 접지 임계(`m_reachSpeedRatio` 1.4)·가중치(0.8)는 눈으로 맞출 값.

**P14 — 사다리 부착 방향 = 면(face) 스냅 + Stop 훅 반복 억제 (2026-08-21)**
- **증상**: 사다리에 붙으면 몸이 옆을 보고 팔다리가 벌어진 채 매달렸다.
- **원인(실측)**: 부착 방향을 <b>플레이어가 접근한 방향 그대로</b> 썼다. 사다리 축은 `right=월드X · up=월드-Z · forward=월드Y`(FBX -90° 회전, 스케일 100)이고
  콜라이더 실측 두께는 `X 2.32 · Y 4.82 · Z 1.56` → **얇은 축(면 법선)은 Z**. 옆(X)에서 다가가면 측면 기둥에 옆으로 붙어버린다.
- **해결**: `Ladder.GetFaceAxes()` — `transform.forward/right` 를 믿지 않고 **콜라이더의 월드 두께**로 판별한다(수평 축 중 얇은 쪽=면 법선, 넓은 쪽=폭 축).
  부착·상단 이탈·그립(IK) 이 전부 이 축을 쓴다. 어느 방향에서 와도 면에 스냅해 정면으로 마주본다.
- **검증(실측)**: 네 방향 접근 → 전부 부착 오프셋 `(0.00, ±0.35)` · 시선 `(0.00, ∓1.00)`(옆 접근 포함) · 그립 X축 ±0.33 · 상단 이탈 반대편.
  회귀 `ClimbTests.옆에서_다가와도_사다리_정면에_붙는다` · **EditMode 213/213 · PlayMode 199/199**(257.1s).
- **동반: Stop 훅 무한 반복 억제** — 정적 리마인더 훅(항상 컨텍스트 주입)이 매 Stop 마다 에이전트를 다시 깨워 빈 응답이 반복됐다.
  ① 정적 리마인더 제거(내용은 CLAUDE.md 에 이미 있음) ② 나머지 가드 3종에 `HOOK_DEDUP` — 같은 경고는 해시가 같으면 침묵(`.claude/hooks/.state/`, gitignore).
  실측: 조건을 만든 뒤 1회차 경고 출력 → 2회차 출력 없음.
...
- **실측**: 트리거 bounds 가 `size (0.82, 0.06, 4.82)` — 높이 4.8m 사다리가 **두께 0.06m 짜리 납작한 판**이 돼 공중(y≈2.04)에 떠 있었다.
  플레이어가 0.18m 앞에 서 있어도 `OverlapSphere` 히트 **0개**.
- **왜**: 사다리 FBX 는 **-90° 회전 + 스케일 100** 으로 들어온다. P6 배선 때 <b>월드 바운즈 크기를 회전 고려 없이 로컬 `BoxCollider.size` 에 그대로</b> 넣어 축이 뒤바뀌었다.
- **수정**: `Ladder.FitColliderToMesh()`(ContextMenu) — 메시 8꼭짓점을 **사다리 로컬 공간으로 변환**해 바운즈를 만든다(회전·스케일 무관).
  수평 패딩(`m_triggerPadding`, 기본 1.5m)은 **월드 m 단위**라 `lossyScale` 로 나눠 로컬로 환산하고, 월드 up 에 가장 가까운 로컬 축(=사다리 길이)은 패딩에서 제외한다.
  (환산을 빼먹어 0.5m 가 50m 가 되는 것도 실제로 밟았다 — 중간 측정 `size (50.82, 54.82, 0.06)`.)
- **결과(실측)**: bounds `size (2.32, 4.82, 1.56)` · y −0.36~4.45 · 탐지=Ladder · 문구="오르기" · Interact→ClimbSensor 부착 · Ground→Climb 신호 True · **화면에 `[E] 오르기` 표시 확인(스크린샷)**.
- **조작 유의**: 몸은 항상 카메라 정면을 향하고 감지는 몸 앞 1m(r 0.5) 에서 일어난다 → **사다리를 화면 정면에 두고** 접근해야 한다(등지면 탐지 0, dot −1.00 실측).
- **검증**: EditMode **213/213** · PlayMode **195/195**(251.5s).
...
- **해결**: Unity 내장 감쇠 `Animator.SetFloat(name, value, dampTime, deltaTime)` 사용 — `CharacterAgentAnimations.SetFloat` 에 감쇠 오버로드 추가,
  `LocomotionSettings.MoveBlendDamp`(기본 0.12s)로 튜닝. 자체 Lerp 대신 내장을 쓰는 이유 = 프레임률 무관 + Animator 평가 시점과 어긋나지 않음.
- **트레이드오프**: 이동은 즉시 꺾이고 애니만 감쇠 시간만큼 따라붙는다 → 그 구간에서만 발/지면이 미세하게 어긋난다(0.12s).
- **검증(실측)**: 좌→우 전환 시 프레임당 블렌드 좌표 변화 **4.6 → 0.216** · 0.5s 내 새 방향 수렴 확인 ·
  회귀 `ActionRootTests.방향을_꺾어도_8방향_파라미터가_한번에_튀지_않는다` · EditMode 212/212 · PlayMode 195/195(1건은 닉네임 중복 플레이크로 재실행 통과).
...
#### 2.86 지속 Effect 를 서버가 소유 + GAS 단일화(클라 GasComponent → Shared AbilitySystemComponent) (2026-08-27)

- **측정이 먼저**: "지속 Effect 만료의 서버 소유"가 절반만 돼 있었다. `EffectTiming.IsExpired` 는 있는데 **프로덕션 호출 0건**(테스트만), `S_RemoveEffect`(Union 1641)는 정의만 있고 **생산처 0건**, 클라 `EffectRemovePacketHandler` 는 **한 번도 안 불렸다**. 만료는 오직 클라 `_active.RemoveAll(...)` 에서만 일어났고 **서버엔 활성 Effect 목록 자체가 없었다.**
- **보안 결과(이게 핵심)**: 서버가 자기가 건 CC 를 몰랐으므로 `AbilitySystemComponent.IsActivationBlocked` 가 스턴 중에도 항상 false 였다. `CombatHandler` 가 그 값으로 `C_Attack` 을 거르는데, **거를 근거가 없었다** — 스턴을 무시하는 클라를 서버가 못 막았다.
- **E1 서버 소유**: Shared 컴포넌트에 활성 목록 + `ApplyEffect(def, instanceId, nowMs, stacks)` + `TickEffects(nowMs)`. `Actor.Tick` 기본 구현이 만료를 돌리고 `ActorTickResult.ExpiredEffectIds` 로 올린다 → `RoomSimulation` 이 `S_RemoveEffect` 로 번역. `RoomSimulation.MonsterAttack` 은 브로드캐스트 **전에** `target.Actor.Gas.ApplyEffect` 로 서버 액터에도 실제로 건다.
  - **태그는 저장하지 않고 파생**: `HasTag` = 직접 부여 ∪ 활성 Effect 의 `GrantedTags`. 회수 장부를 두면 같은 스턴 2개 중 하나가 끝날 때 잘못 떼고, 그 순간 영구 스턴이 된다. 파생값이면 만료가 곧 해제라 어긋날 여지가 없다.
  - **만료 반환은 `IReadOnlyList<int>?`(없으면 null)** — 만료는 드문 사건인데 10Hz × 액터 수만큼 빈 리스트를 할당할 이유가 없다.
  - **배선 버그 하나 잡음**: 스택 정책이 기존 인스턴스를 재사용하면 방금 뽑은 id 는 버려진다. 그걸 그대로 브로드캐스트하면 만료 통지 id 와 짝이 어긋나 **클라에 CC 가 영영 남는다** → `ApplyEffect` 의 **반환값**을 싣도록 수정.
- **E2 이름 스왑**: Shared 로직 = `AbilitySystemComponent`(구 `GasComponent`), 클라 MonoBehaviour = `GasComponent`(구 `AbilitySystemComponent`). 둘이 **같은 네임스페이스**(`Script.System.GamePlayAbilitySystem`)라 한쪽만 바꾸면 중복 정의 → 양쪽 동시 수정. `.meta` GUID(`e61c464a…`) 보존해 프리팹 4곳(`PlayerCharacter`·`RemotePlayerCharacter`·`Main.unity`·`_Recovery/0.unity`) 참조 유지.
- **E3 완전 단일화**(사용자 선택 — 부분 위임 대신): 런타임 상태를 **전부** Shared 가 소유한다.
  - `GameplayAttribute` → **저작 전용 DTO**. 필드(`_attributeType`·`_kind`·`_baseValue`·`_maxValue`·`_currentValue`)는 프리팹 YAML 바인딩 때문에 그대로 두고, 런타임 API(`OnChanged`·`SetCurrent`·`SetMax`·`ApplyModifier`)는 제거. `Awake` 가 `DefineResource`/`DefineStat` 로 Shared 에 옮긴다.
  - `AttributeSet` += `Base`·`IsStat` → 스탯 재계산이 **Base 기준**(클라 `RecalculateStats` 와 동일 산식). 자원(HP·마나)은 재계산 대상이 아니다 — 되돌리면 피해가 사라져 불사가 된다.
  - `GetAttribute()`(속성 객체 노출) 제거 → `Current`/`Max`/`Has`/`SetCurrent`/`SetMax`/`ApplyModifiers`(값만 오감). 예전엔 호출부가 받아 간 객체에 대고 직접 써서 **이 컴포넌트가 변화를 놓치는 경로**(이벤트 미발행)가 있었다. 이제 모든 변경이 여기를 지난다.
  - `OnAttributeChanged` 는 **변경 후 diff** 로 재발행. Shared 는 이벤트를 갖지 않는다 — 락 안에서 콜백을 부르면 그 콜백이 다시 락을 잡는 순간 설계가 무너진다.
  - `ActiveGameplayEffect.cs` 를 클라 → `Shared.Gameplay/Effects/` 로 이동(순수 C#, 엔진 의존 0). 클라는 Plugins DLL 로 같은 타입을 쓴다(csproj `CopyToUnityPlugins` 가 자동 복사).
- **갈라져 있던 의미 하나 통일**: 클라는 `EStackPolicy.None` 을 "재적용 무시"로 구현·**테스트로 문서화**했는데 Shared 는 새 인스턴스를 추가했다 → 같은 효과를 두 번 걸면 클라 1개 / 서버 2개. **클라 쪽(의도적·테스트된 쪽)으로 통일**.
- **파일**: `Shared.Gameplay/Actors/{AbilitySystemComponent,AttributeSet}.cs` · `Shared.Gameplay/Effects/ActiveGameplayEffect.cs`(이동) · `SocketServer/Actors/{Actor,PlayerActor,MonsterActor}.cs` · `SocketServer/Room/RoomSimulation.cs` · `SocketServer/Combat/CombatEffectCatalog.cs`(`Get` 추가) / 클라 `System/GameplayAbilitySystem/{GasComponent,Attribute/GameplayAttribute,Effects/GameplayEffect}.cs` + 호출부 7(`PlayerCharacterAgent`·`CharacterSpawner`·`PlayerStatApplier`·`EffectReceiver`·`InGameModel`·`PartyModel`).
- **검증**: `ServerAll.sln` 0오류 · `Shared.Gameplay.Tests` **90/90**(74→90, 신규 16 = `AbilitySystemComponentActiveEffectTests`) · `SocketServer.Tests` **307/307**(304→307, 신규 3 = CC 부여·만료·미만료) · `GameServer.Tests` 457/457 · Unity 컴파일 0오류 · **EditMode 260/262 · PlayMode 242/242(Docker 리빌드 후 E2E 포함)**. EditMode 실패 2건은 미커밋 채팅 DI 작업(`RootContainerResolveTests` — `IGameSceneManager` 미등록)으로 이 변경과 무관.
