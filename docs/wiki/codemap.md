...
- **위치**: `GUI/Shop/Shop.cs`(ShowToast 실패→`ShowFailPopupAsync`, 미사용 ToastFail 제거) · `Presentation/Shop/ShopModel.cs`(메시지). 검증 CS0.
- **잔여(미적용)**: 인벤토리 판매(Sell)·소비·장착 피드백은 `InventoryModel.OnToast`가 **string-only(성공/실패 플래그 없음)** + Inventory View가 `Debug.Log`만 → 동일 가시화하려면 토스트 타입을 flag 포함으로 소폭 refactor 필요(별도 작업). ※`necklace_vitality x2` 가 1005로 실패하는 **근본 원인**(구매 불가 아이템/조건)은 UX와 별개로 확인 대상.
