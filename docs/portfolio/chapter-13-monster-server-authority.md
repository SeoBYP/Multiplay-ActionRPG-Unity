...
이 챕터 시점엔 플레이어 HP가 클라 결정론이었다. 몬스터도 같은 방식으로 클라가 시뮬하면 통일성은 있지만, 클라가 몬스터 HP/위치를 시뮬하면 클라마다 다른 몬스터를 보게 되고(일관성 붕괴) 치팅 표면도 커진다. 그래서 우선 몬스터부터 서버 권위로:

| 대상 | HP 권위 (이 챕터 시점) | 이동 |
|------|---------|------|
| 플레이어 | 클라 결정론 → **이후 던전 HP 서버 권위로 승격**(2.5.1) | 클라 입력 → 서버 릴레이 |
| 몬스터 | **서버 권위** | **서버 시뮬 + 클라 보간** |

몬스터는 "서버가 소유한 객체"라 권위를 서버로 일원화하는 게 자연스럽다. 클라엔 **몬스터 AI도 물리도 없다** — 받은 스냅샷을 보간만 한다.

> **정정(이후 진화)**: "플레이어 HP=클라 결정론"은 이 챕터 시점의 상태였고, 곧 **부채**로 드러났다 — 던전에서 모든 데미지의 출처가 서버인데 HP만 클라가 가지면 **불사 핵**(C_PlayerDead 미송신/S_ApplyEffect 무시)이 가능했다. 그래서 2.5.1에서 **던전 플레이어 HP도 서버 권위로 승격**했다(서버가 데미지 누적→HP0 직접 감지→S_PlayerDead). 즉 이 챕터의 "의도된 비대칭"은 *몬스터 우선*이라는 단계적 결정이었지, 플레이어 HP를 영구히 클라에 둔다는 뜻은 아니었다. 정본 = [authority-model.md §4](../wiki/authority-model.md).
...
| 키워드 | 한 줄 설명 |
|--------|-----------|
| 의도된 비대칭(단계적) | 몬스터 HP=서버 권위 / 플레이어 HP=클라 결정론(이 챕터 시점) → **이후 2.5.1에서 던전 플레이어 HP도 서버 권위 승격**(불사 핵 차단, authority-model §4) |
