...
- **애니메이션**: MotionMatching은 별도 프로젝트로 외부화(범위 관리) — 클라는 Animator 실배선으로 완성도 확보.
  **ARPGWarrior 전환(2026-08-20~21)**: 아바타 타입 불일치(Generic 클립 → Humanoid 메시)로 배선 완료 상태에서 본 회전 0.0도였던 것을 실측으로 잡고, 컨트롤러를 **코드 생성**(상태 15·파라미터 19·18모션 블렌드)으로 재구축. 발 슬라이딩은 블렌드 좌표를 **클립 실측 m/s 공간**으로 바꿔 소멸(플레이어·몬스터 비율 1.00), 4단 콤보(타이밍 진실원 = 스킬 데이터)·8방향 회피·사다리(로컬 전용, 최소개입 IK)·상호작용 프롬프트 UI. → [챕터 28](./chapter-28-animation-retarget-units-ik.md) · 배선 상세 [player-animation-arpg.md](../wiki/player-animation-arpg.md)
- **멀티플레이 동기화(2026-08-22)**: 던전 원격 플레이어 애니 동기화(방향은 위치·회전에서 **역산**, 모드만 1바이트, 회피 방향은 이벤트에) + 그 과정에서 드러난 결함 셋 — 아군 오사가 클라 HP만 깎아 **서버가 사망을 감지 못하던 문제**(→ 오사 제거), Play 정지 후 **재입장 30회 거절**(→ 재접속 인수, 원인은 감지에 63초 걸리는 죽은 세션), 내 수정이 만든 **Walk/Run 미재생 회귀**(→ 전이 즉시화 + "파라미터가 아니라 재생을 검증"하는 테스트 신설). → [챕터 29](./chapter-29-multiplayer-sync-invisible-failures.md)
