...
### 클리어/실패를 단순 bool 플래그로? — 동시성 1회 보장

**내가 처음 만든 것:**
```csharp
if (!room.IsCleared) { room.IsCleared = true; Broadcast(S_DungeonClear); }
```

**피드백:**
틱 스레드와 패킷 핸들러가 동시에 이 분기에 들어오면 **두 번 발화**할 수 있다. 또 "전멸=클리어"와 "전원 다운=실패"가 **거의 동시에** 성립할 수 있는데, bool 두 개면 둘 다 켜질 수 있다.

**올바른 설계 — Interlocked 단일 outcome:**
```
...
**내가 처음 생각한 것:**
전멸을 감지한 SocketServer가 그 자리에서 DB에 Exp를 써버린다.

**피드백:**
SocketServer는 **실시간 전용**(TCP/틱). 영속·도메인(Progression)은 **GameServer 책임**이다. 그리고 두 서버는 **직접 RPC 금지** — Redis Streams로만 통신.
...
### Exp를 UserProfile 컬럼에 넣을까 — 별도 도메인 분리

**내가 처음 생각한 것:**
`users` 테이블에 `level`, `exp` 컬럼 추가.

**피드백:**
미래에 **캐릭터 교체(원신식)** 를 넣으면 Exp/Level은 계정이 아니라 **캐릭터 귀속**이 되어야 한다. UserProfile에 박으면 그때 마이그레이션 지옥.
...
```
완전한 Co-op 1판 루프 시각 검증 (MPPM 2-client) — 클리어→보상→로비 전체 통과
