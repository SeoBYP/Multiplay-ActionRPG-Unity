...
### 실패라고 다 같은 실패가 아니다 — 기준은 "소유를 증명했는가"

```
소유 증명 실패(위조 문자열·다른 기기) →  실패만 반환, 세션 유지     ← 증명이 없으면 아무것도 파괴하지 않는다
구세대 토큰 + 유예 이내               →  정상 회전                  ← 응답을 못 받은 클라이언트의 재시도
구세대 토큰 + 유예 경과               →  RefreshToken 삭제 + 세션 제거  ← 탈취 확정
리프레시 토큰 자연 만료               →  실패만 반환, 세션은 건드리지 않음 ← 정상 사용자
```

**파괴는 소유 증명 뒤에만 온다.** 초기 구현은 반대였다 — 버전 역행 검사(`submittedVersion < RefreshTokenVersion`)가 해시 비교보다 앞에 있어서, 유출된 만료 Access Token 하나만 있으면 `aaa.0` 같은 아무 문자열로 피해자 세션을 끊을 수 있었다(8절 참조). 지금은 제출된 토큰의 해시가 **현재 세대 또는 직전 세대와 일치**할 때만 그 다음 판단으로 넘어간다. 버전 정수는 더 이상 파괴의 근거가 아니다.

직전 세대 해시는 Redis 전용 휘발성 키(`game:user:credential:refresh:prev:{userId}`, 값 = `해시:회전시각`)에 둔다. DB 컬럼을 늘리지 않은 이유는 이것이 **탐지용 신호**라서다 — 캐시가 만료돼 신호를 잃으면 탐지가 "그냥 실패"로 강등될 뿐, 잘못된 파괴로 이어지지 않는다.

유예창(`RefreshReuseGraceSeconds`, 기본 60초)이 필요한 이유는 공격자가 아니라 **정상 클라이언트** 때문이다. 응답이 유실되거나 타임아웃 재시도가 겹치면 같은 토큰이 두 번 도착하는데, 그것을 탈취로 판정하면 대량 로그아웃이 된다. 유예 안의 재시도는 회전시각을 갱신하지 **않는다** — 갱신하면 같은 토큰을 계속 되밀어 탐지를 무한히 미룰 수 있다.

원본 학습 로그에는 *"검증 순서를 null → 만료일 → 해시 비교로 바꿀 것"* 이라는 TODO가 있었다. **만료 검사를 앞으로 당기는 부분은 지금도 따르지 않는다.** 아무 문자열이나 던진 공격자에게 "이 계정의 토큰은 만료됐다"는 정보를 주기 때문이다. 소유 증명을 통과한 뒤에 상태(만료)를 알려주는 순서를 유지한다.
...
내부:  "Refresh failed because session {SessionId} was not found"
       "Refresh failed because refresh token did not match any issued token for user {UserId}"
       "Refresh token reuse detected for user {UserId}. Rotated {ElapsedSeconds}s ago"
       "Refresh accepted previous-generation token as a retry for user {UserId} ({ElapsedSeconds}s after rotation)"
```
...
`GameServer.Tests/Application/Services/AuthServiceTests.cs` — 15케이스. 해피패스보다 **실패 경로**가 많다.

| 검증하는 것 | 테스트 |
|---|---|
| 재사용 탐지 → 세션 종료 | `ReuseRefreshToken_유예_경과_후_구버전_토큰_재사용은_세션을_종료한다` |
| 재시도 유예 → 세션 유지 | `ReuseRefreshToken_유예_시간_내_중복_요청은_재시도로_처리된다` |
| 위조 문자열로 파괴 불가 | `RefreshToken_위조된_리프레시_문자열로는_세션을_파괴할_수_없다` |
| 기기 바인딩 실패(세션 유지) | `RefreshToken_다른_기기가_제출하면_실패하지만_세션은_유지된다` |
...
저장 계층은 `UserCredentialRepositoryIntegrationTests.직전_세대_리프레시_토큰_기록은_해시와_회전시각을_그대로_되돌린다`(실 Redis)가, 실서버 경로는 `AuthE2ETests`의 `Refresh_위조된_리프레시_문자열은_세션을_끊지_못한다` · `Refresh_잘못된_DeviceId_실패해도_정상_기기는_갱신된다`가 덮는다.
...
### ✅ 해소한 결함 — 세션 파괴가 DoS 벡터였다 (2026-08-23)

갱신에는 `accessToken`이 필요하지만 **만료된 것도 통과**한다(6절). 그래서 유출된 만료 Access Token 하나만 있으면 공격자가 형식만 맞춘 아무 리프레시 문자열(`aaa.0`)로 **피해자의 세션을 강제 종료**시킬 수 있었다. 반복하면 지속적 로그아웃이다.

조사하면서 **문제가 알려진 것보다 한 겹 더 깊다**는 것을 알았다. 흔한 처방인 "전체 무효화를 재사용 탐지로 한정한다"(OAuth 2.0 Security BCP)를 그대로 적용해도 여기서는 DoS가 남는다 — 이 코드의 재사용 탐지가 **공격자가 자유롭게 고르는 정수 하나**(토큰 접미사 버전)에 기대고 있었고, 그 검사가 해시 비교보다 **앞**에 있었기 때문이다.

```
before:  버전 역행 검사(위조 가능) → 파괴  ────────▶ 해시 비교에 도달조차 안 함
after :  해시 = 현재 세대?  ──▶ 정상 회전
         해시 = 직전 세대?  ──▶ 유예 이내 = 재시도 / 유예 경과 = 탈취 → 파괴
         둘 다 아님        ──▶ 실패만. 세션 유지
```

직전 세대 해시를 Redis에 한 세대만 남겨 **"우리가 발급한 토큰"이라는 증명**을 되찾았기 때문에, 가용성과 재사용 탐지 중 하나를 고를 필요가 없어졌다. 처음에 "가용성 vs 보안"으로 보였던 선택지는 실은 **검사 순서 문제**였다.
