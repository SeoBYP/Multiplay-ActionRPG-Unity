...
namespace Game.System.MotionSystem
{
    public class RecordPositions : MonoBehaviour
    {
        ...
        private bool _startedEditorAnimationMode;
        ...
        {
            Tags tags = ScriptableObject.CreateInstance<Tags>();
            tags.tags = baseTags;
        ...
        {
            var basePath = "Assets/GameResources/MotionMatching/Player";
            try
            {
            ...
            var path = basePath + "/PlayerMotionDataset.asset";
            _dataset = SaveOrOverwriteAsset(_dataset, path);
        ...
        {
            basePath += "/PlayerCharacteristics.asset";
            _dataset.characteristics = SaveOrOverwriteAsset(characteristics, basePath);
        ...
        {
            basePath += "/PlayerQueriesComputed.asset";
            _dataset.queriesComputed = SaveOrOverwriteAsset(queries, basePath);
            
            EditorUtility.SetDirty(_dataset
                .queriesComputed); // This ensures the modified data in the scriptable object is saved
        }

        private void SaveTags(Tags tags, string basePath, string datasetName)
        {
            basePath += "/PlayerTags.asset";
            _dataset.tagsList = SaveOrOverwriteAsset(tags, basePath);
        ...
        {
            basePath += "/PlayerHumanoidAvatar.asset";
            if (_avatar is HumanoidAvatar humanoidAvatar)
            {
                //Humanoid Avatar
                _dataset.avatar = SaveOrOverwriteAsset(humanoidAvatar, basePath);
            }
            else if(_avatar is GenericAvatar genericAvatar)
            {
                //Generic Avatar
                _dataset.avatar = SaveOrOverwriteAsset(genericAvatar, basePath);
            }
        ...
        private static T SaveOrOverwriteAsset<T>(T source, string assetPath) where T : UnityEngine.Object
        {
            ...
            if (existing != null)
            {
                if (ReferenceEquals(source, existing))
                    return existing;
        ...
        private void FixedUpdate()
        {
            ...
            if (_isNextAnimation)
            {
                ...
                SaveDataset();
                StopEditorAnimationSampling();
                return;
            }

            if (!_animationFinished)
            {
                SampleCurrentAnimationInEditor();
                    ...
                    currentTag.ranges
                        ...
                        .ForEach(range =>
                        {
                            ...
                            var stopFramesInFeatureSpace =
                                (int)Math.Ceiling(((ResolveFrameStop(range, totalFrames) * totalFramesInFeatureSpace) / totalFrames));
                ...
                foreach (var currentTag in _dataset.tagsList.actionTags)
                {
                    currentTag.ranges
                        ...
                        .ForEach(range =>
                        {
                            ...
                            var stopFramesInFeatureSpace =
                                (int)((ResolveFrameStop(range, totalFrames) * totalFramesInFeatureSpace) / totalFrames);
        ...
        private static float ResolveFrameStop(TagRange range, float totalFrames)
        {
            return range.frameStop == int.MaxValue ? totalFrames : range.frameStop;
        }
        ...
        private void SampleCurrentAnimationInEditor()
        {
            if (Application.isPlaying || _currentAnimation == null || _animator == null)
                return;

            if (!AnimationMode.InAnimationMode())
            {
                AnimationMode.StartAnimationMode();
                _startedEditorAnimationMode = true;
            }

            AnimationMode.BeginSampling();
            AnimationMode.SampleAnimationClip(_animator.gameObject, _currentAnimation, _lengthAnimationRecorded);
            AnimationMode.EndSampling();
        }

        private void StopEditorAnimationSampling()
        {
            if (!_startedEditorAnimationMode || !AnimationMode.InAnimationMode())
                return;

            AnimationMode.StopAnimationMode();
            _startedEditorAnimationMode = false;
        }
                ...
                SetFeaturePastDirections(
                    ...
                    math.normalize(
                        math.mul(inverseModel, 
                            new float4(math.normalize(math.mul(globalFrameRotation, Float3Extensions.Forward)), 0.0f)).xyz)
                    );
            }
        }
