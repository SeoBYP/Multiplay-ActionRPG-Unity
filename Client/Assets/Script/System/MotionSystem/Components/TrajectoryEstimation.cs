...
{
    /// <summary>
    /// 입력/AI intent를 기반으로 미래 trajectory와 과거 trajectory를 관리하는 컴포넌트입니다.
    /// Motion Matching 검색은 단순 현재 속도만 보지 않고, 앞으로 어디로 갈지와 방금 어디서 왔는지를 같이 비교합니다.
    /// </summary>
    public class TrajectoryEstimation
    {
        ...
        /// <summary>
        /// future/past sample 개수와 responsiveness를 기준으로 예측 버퍼를 초기화합니다.
        /// </summary>
        ...
        /// <summary>
        /// 현재 입력, 바라볼 방향, query별 속도 설정을 사용해 future trajectory를 계산합니다.
        /// 결과는 FuturePrediction에 저장되며 PoseFinder의 future feature 비교에 사용됩니다.
        /// </summary>
        ...
        /// <summary>
        /// 현재 root position/forward를 ring buffer에 기록하고, PoseFinder가 사용할 past trajectory sample을 갱신합니다.
        /// </summary>
        ...
        /// <summary>
        /// 외부에서 전달된 과거 trajectory로 내부 buffer를 채웁니다.
        /// 액션 시작 시 과거 움직임을 강제로 맞추는 용도로 확장할 수 있습니다.
        /// </summary>
        ...
        //Debug gizmos
        /// <summary>
        /// 현재 예측 trajectory, baked trajectory, bone characteristic을 Scene view에 표시합니다.
        /// 검색 품질을 디버깅할 때 사용합니다.
        /// </summary>
        ...
        private void DebugRootProperties(float height, Transform root)
        {
            ...
            GizmosExtensions.DrawArrow(new float3(root.transform.position.x, height, root.transform.position.z),
        ...
        private void DebugTrajectoryPredictions(float height, float3 position, FuturePrediction nextPrediction)
        {
            ...
            for (int i = -1; i < nextPrediction.futurePositions.Length - 1; i++)
            {
                ...
                Gizmos.color = Color.yellow;
                GizmosExtensions.DrawArrow(new float3(C.x, height, C.y), nextPrediction.futureDirections[i + 1] * 0.3f, 0.15f);
        ...
        private void DebugTrajectoryPredictions(float height, Transform root, FuturePrediction nextPrediction)
        {
            ...
            for (int i = -1; i < nextPrediction.futureOffsets.Length - 1; i++)
            {
                ...
                GizmosExtensions.DrawArrow(new float3(C.x, height, C.y), dirInGlobalSpace * 0.3f, 0.15f);
        ...
        private void DebugPastTrajectory(float height, float3 position, PastTrajectory pastPoses)
        {
            ...
            for (int i = -1; i < pastPoses.pastGlobalPosition.Length - 1; i++)
            {
                ...
                //Gizmos.color = Color.green;
                GizmosExtensions.DrawArrow(new float3(C.x, height, C.y), pastPoses.pastGlobalDirection[i + 1] * 0.3f, 0.15f);
        ...
        /// <summary>
        /// 생성한 NativeArray와 trajectory buffer를 해제합니다.
        /// </summary>
        public void Destroy()
        {
