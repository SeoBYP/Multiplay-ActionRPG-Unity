...
{
  /// <summary>
  /// 하나의 QueryComputed를 runtime에서 실행하는 흐름 객체입니다.
  /// QueryComputed는 baked feature data이고, QueryComputedFlow는 현재 feature index, PoseFinder, PoseSetter,
  /// distance buffer, blending 진행 시간 같은 런타임 상태를 관리합니다.
  /// </summary>
  public abstract class QueryComputedFlow
    {
        ...
        public NativeArray<int> characterTransformBoneIndices;
        public CurrentBoneTransformsValues currentBoneTransformsValues;
        public Transform root;
        public float searchRate;
        public float animationSwitchPenalty;
            ...
            float animationSwitchPenalty)
        {
            ...
            this.animationSwitchPenalty = animationSwitchPenalty;
            this.characterTransformBoneIndices = characterTransformBoneIndices;
        ...
        /// <summary>
        /// query data를 연결하고, bone weight와 distance buffer 등 runtime 검색 준비를 수행합니다.
        /// </summary>
        public abstract void Build(QueryComputed queryComputed, int length);
        
        /// <summary>
        /// query 시작 시 현재 feature와 PoseFinder search counter를 초기화합니다.
        /// </summary>
        ...
        /// <summary>
        /// poseStep마다 호출되어 현재 motion data를 갱신하고, PoseFinder로 다음 feature를 선택한 뒤,
        /// 선택된 pose로 전환하기 위한 BlendBoundaries를 초기화합니다.
        /// </summary>
        ...
        /// <summary>
        /// 선택된 feature의 현재/다음 AnimationData를 읽어 root/bone blend boundary를 준비합니다.
        /// search 전환일 경우 inertialization offset과 previous root-based position도 함께 갱신합니다.
        /// </summary>
            ...
            bool isForcedContinuous = false)
        {
            ...
            new InitializeBoneTransitionsJob
            {
                ...
                wantApplyRootBonePosition = wantApplyRootBonePosition
            }.Schedule(currentBoneTransformsValues.bonesCounter, batchCount).Complete();
        }

        /// <summary>
        /// query tag별 Characteristics 설정을 합산해 bone/future/past distance weight를 만듭니다.
        /// </summary>
        ...
        /// <summary>
        /// BlendBoundaries와 MotionData offset을 기반으로 이번 frame에 적용할 bone/root 결과를 계산합니다.
        /// </summary>
        ...
        /// <summary>
        /// 계산된 root/bone 결과를 실제 CharacterController와 TransformAccessArray에 적용합니다.
        /// </summary>
            ...
            )
        {
            ...
            new SetBonesJob
            {
                ...
                wantApplyScales = wantApplyScales,
                wantApplyRootBonePosition = wantApplyRootBonePosition,
                boneIndices = characterTransformBoneIndices
        ...
        private struct CalculateBonesValuesJob : IJobParallelFor
        {
            ...
            {
                bool applyBonePosition = index != rootNode
                    ? wantApplyPositions
                    : wantApplyRootBonePosition;
            ...
            public void Execute(int index)
            {
                ...
                if (isInertialized && isSearch)
                {
                    bool applyBonePosition = index != rootNode
                        ? wantApplyPositions
                        : wantApplyRootBonePosition;
                    Inertialization.InitBoneTransition(ref motionData, index, bonesData, rootNode, applyBonePosition, wantApplyScales);
                }
