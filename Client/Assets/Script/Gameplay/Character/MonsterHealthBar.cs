...
{
    /// <summary>
    /// 몬스터 머리 위 월드공간 체력바(연출 전용). 부모 <see cref="MonsterEntity"/> 의 HpChanged 를 구독해
    /// fill(Image, Filled Horizontal)의 fillAmount 를 Hp/MaxHp 로 갱신하고, 매 프레임 카메라를 향해 빌보드한다.
    ///
    /// HP 진실원 = 서버 권위(S_MonsterState). 이 컴포넌트는 표시만 하며 판정에 관여하지 않는다.
    /// </summary>
    public sealed class MonsterHealthBar : MonoBehaviour
    {
        ...
        private void Awake()
        {
            _monster = GetComponentInParent<MonsterEntity>();
        }
        ...
        private void Refresh(MonsterEntity monster)
        {
