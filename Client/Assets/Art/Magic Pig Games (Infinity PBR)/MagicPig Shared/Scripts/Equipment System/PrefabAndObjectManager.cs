...
*/
        ...
        public int GroupIsActive(PrefabGroup group)
        {
            ...
            var objects = group.groupObjects;
            for (int i = 0; i < objects.Count; i++)
            {
                var obj = objects[i];

                if (obj.render && VariableConditionsMet(obj))
                    renderableObjects++;

                if (obj.isPrefab)
                {
                    if (obj.inGameObject != null) liveObjects++;
                }
                else if (obj.objectToHandle != null && obj.objectToHandle.activeSelf)
                {
                    liveObjects++;
                }
            }
